...
const MODEL_COSTS: Record<string, ModelCost> = {
  // Free/Local models
  'ollama/qwen2.5-coder:7b': { provider: 'ollama', model: 'qwen2.5-coder:7b', costPer1kTokens: 0, outputCostPer1kTokens: 0, tier: 'free' },
  'ollama/qwen2.5-coder:32b': { provider: 'ollama', model: 'qwen2.5-coder:32b', costPer1kTokens: 0, outputCostPer1kTokens: 0, tier: 'free' },
  ...
  // Medium cost
  'anthropic/claude-3-5-haiku-20241022': { provider: 'anthropic', model: 'claude-3-5-haiku-20241022', costPer1kTokens: 0.25, outputCostPer1kTokens: 1.25, tier: 'medium' },
  'openai/gpt-4o': { provider: 'openai', model: 'gpt-4o', costPer1kTokens: 2.5, outputCostPer1kTokens: 10, tier: 'medium' },
...
export class BudgetManager {
    ...
    switch (this.currentMode) {
      case 'normal':
        ...
        if (taskType === 'code' || taskType === 'debug') {
          return { provider: 'ollama', model: 'qwen3-coder:480b-cloud', reason: 'Best for code tasks' };
        }
        return { provider: 'anthropic', model: 'claude-sonnet-4-20250514', reason: 'Best for complex tasks' };
        
      case 'cost_saving':
        // Use cheaper but still capable models
        if (taskType === 'code' || taskType === 'debug') {
          return { provider: 'ollama', model: 'qwen3-coder:480b-cloud', reason: 'Cost-saving: Good code model' };
        }
        return { provider: 'openai', model: 'gpt-4o-mini', reason: 'Cost-saving: Capable but cheaper' };
        
      case 'critical':
        // Use cheapest models
        if (taskType === 'code' || taskType === 'debug') {
          return { provider: 'ollama', model: 'qwen2.5-coder:32b', reason: 'Critical budget: Free local model' };
        }
        return { provider: 'ollama', model: 'qwen3-coder:480b-cloud', reason: 'Critical budget: Low-cost cloud model' };
    }
  }
   ...
   */
  isFeatureAvailable(feature: 'images' | 'parallel_agents' | 'deep_analysis' | 'long_context'): boolean {
    switch (this.currentMode) {
      case 'normal':
