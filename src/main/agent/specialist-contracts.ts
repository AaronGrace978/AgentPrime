...
const ALL_WORKSPACE_GLOBS = ['**/*'] as const;
const FRONTEND_APP_GLOBS = ['src/**', 'app/**', 'pages/**', 'lib/**', 'components/**'] as const;
const PROJECT_METADATA_GLOBS = [
  'package.json',
  'package-lock.json',
  'vite.config.*',
  'next.config.*',
...
const BACKEND_GLOBS = ['backend/**', 'scripts/**'] as const;
const TEST_GLOBS = ['tests/**'] as const;
const PUBLIC_ASSET_GLOBS = ['public/**'] as const;
...
export const SPECIALIST_MATRIX: Record<SpecialistId, SpecialistDefinition> = {
  executive_router: {
    ...
    writableGlobs: [],
    readableGlobs: ALL_WORKSPACE_GLOBS,
  ...
  python_specialist: {
    ...
    readableGlobs: [...BACKEND_GLOBS, ...TEST_GLOBS, 'requirements*.txt', 'pyproject.toml'],
