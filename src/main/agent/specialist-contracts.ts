...
export const SPECIALIST_MATRIX: Record<SpecialistId, SpecialistDefinition> = {
  ...
  javascript_specialist: {
    ...
    writableGlobs: ['src/**/*.js', 'src/**/*.jsx', 'src/**/*.ts', 'src/**/*.tsx', 'tests/**/*.ts', 'tests/**/*.tsx'],
    readableGlobs: ['src/**', 'tests/**', 'package.json', 'tsconfig*.json', 'vite.config.*'],
  ...
  repair_specialist: {
    ...
    writableGlobs: ['src/**', 'backend/**', 'tests/**', 'package.json', 'vite.config.*', 'tsconfig*.json'],
    readableGlobs: ['**/*'],
    allowedToolNames: ['read_file', 'write_file', 'patch_file', 'search_codebase', 'find_symbols'],
    allowedCommandPrefixes: [],
