...
class TemplateEngine {
    ...
    loadRegistry() {
        ...
        try {
            ...
            const templateCount = this.registry.templates ? this.registry.templates.length : 0;
            const categoryCount = this.registry.categories ? this.registry.categories.length : 0;
     ...
     */
    async createProject(templateId, targetDir, variables) {
     ...
     */
    async installNodeDependencies(projectPath) {
        const packageJsonPath = path.join(projectPath, 'package.json');
     ...
     * Generate .bat launcher files from package.json scripts
     */
    generateBatFiles(projectPath, variables) {
        const createdBats = [];
        const packageJsonPath = path.join(projectPath, 'package.json');
...
`;
     ...
     */
    createBatFile(scriptName, scriptCommand, variables) {
...
`;
     ...
     */
    createRunBat(scripts, variables) {
        const projectName = variables.projectName || 'project';
        let menu = `@echo off
...
if errorlevel 1 (
     ...
     * Generate .bat files for Python projects
     */
    generatePythonBatFiles(projectPath, variables) {
        const createdBats = [];
        const requirementsPath = path.join(projectPath, 'requirements.txt');
