...
- **Native tool-calling across all providers:** `src/main/ai-providers/anthropic-provider.ts`, `src/main/ai-providers/openai-provider.ts`, and `src/main/ai-providers/openrouter-provider.ts` now expose a real `chatWithTools(...)` path that talks to each provider's first-party tool API (Anthropic Messages API `tools`, OpenAI Chat Completions `tool_calls` for GPT-4.x and the new Responses API `function_call` for GPT-5.x, and the OpenAI-compatible OpenRouter pipe). Ollama already had it; the router's capability matrix in `src/main/ai-providers/index.ts` now reflects that all four providers are natively tool-capable instead of only Ollama.
- **Shared canonical tool-format adapter:** New `src/main/ai-providers/tool-format.ts` and `src/main/agent/canonical-tools.ts` translate the agent loop's local tool catalog into the canonical Anthropic-style schema and back to the agent loop's `ParsedToolCall` shape. The agent loop now calls `aiRouter.chatWithTools(...)` and prefers `response.toolCalls` over the legacy regex/JSON-in-text parser, which is now a strict fallback path.
- **Universal request cancellation via `AbortSignal`:** `ChatOptions` in `src/types/ai-providers.d.ts` now carries an optional `signal: AbortSignal` that is threaded through every provider's HTTP layer (`chat`, `stream`, and `chatWithTools` for Anthropic, OpenAI, OpenRouter, and Ollama). Aborts immediately tear down the underlying axios socket so model billing stops the moment the user clicks Stop, instead of waiting for the next agent iteration boundary.
- **Anthropic streaming hardened:** While in there, `stream()` was rebuilt on the same canonical message normalizer as `chat()` / `chatWithTools()` — system message joining, empty-content filtering, consecutive same-role merging, and proper `\n\n` SSE event buffering. It also emits a clean error chunk on abort/transport failure instead of swallowing it.
- **Per-task `AbortController` in the agent loop:** `src/main/agent-loop.ts` now creates a controller on each `run()` and aborts it from `requestStop()`, so the existing UI Stop button cancels the live model HTTP request instead of just flipping a between-iterations flag.
- **End-to-end abort wiring in chat + OpenAI-compat APIs:** `src/main/ipc-handlers/chat.ts` keeps a per-request `AbortController` map for in-flight chat streams, threads the signal into both `dualStream` and `stream`, and the `agent:stop` IPC now aborts active chat streams in addition to stopping the agent. `src/main/inference-server/index.ts` ties every public endpoint (`/v1/chat/completions`, `/api/chat`, `/api/generate`) to `req.on('close')` so external client disconnects cancel the upstream provider call.
- **Typed `AbortError` short-circuits the fallback chain:** `src/main/core/timeout-utils.ts` introduces a real `AbortError` class plus `isAbortError(...)`, and `withRetry` / `withSmartFallback` now bail out immediately on it. Previously, clicking Stop would throw a generic `Error('Request aborted')` that the smart-fallback path treated like a transient failure and silently kicked off the entire `FALLBACK_MODEL_CHAIN` against every model in the lineup. The router's `shouldFallbackFromError` also short-circuits on `AbortError`, and every provider's `chat` / `chatWithTools` catch block now translates axios `CanceledError` into `AbortError` instead of returning `{ success: false }`.
- **Friendly stopped state in the chat UI:** The chat IPC handler now emits `chat-stream { aborted: true }` (instead of `error: 'Request aborted'`) when the user cancels, and the outer catch returns `{ success: true, aborted: true }` so the renderer can render a calm "Stopped" pill instead of a scary error toast.
